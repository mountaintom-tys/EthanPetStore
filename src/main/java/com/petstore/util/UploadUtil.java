...
public class UploadUtil {
     ...
     * @throws Exception
     */
    public static String fileUpload(MultipartFile file,String contextPath) throws Exception {
        // 判断是否有上传文件
        if (Objects.isNull(file) || file.isEmpty() || Objects.isNull(file.getOriginalFilename())) {
            return null;
        }
        String savePath = "/static/upload"; // 保存文件的相对目录
        String fileName = file.getOriginalFilename();
        // 文件存储路径
        String path = contextPath+ savePath;
        // 获取当前文件类型
